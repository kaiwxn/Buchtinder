...
function Profile() {
  ...
  const [profileBackup, setProfileBackup] = useState<ProfileData | null>(null);
  ...
  const [avatarUrl, setAvatarUrl] = useState("https://via.placeholder.com/100?text=Default");
  ...
  const avatarModal = (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-30 backdrop-blur-lg">
      <div className="bg-white rounded-lg p-4 shadow-lg w-full max-w-md">
        <h2 className="text-2xl font-bold mb-4 text-center">Avatar auswählen</h2>
          ...
          {availableAvatars.map((url) => (
            <img
              ...
              className={`w-24 h-24 rounded-full cursor-pointer border-4 ${
                tempAvatar === url ? "border-blue-600" : "border-transparent"
              }`}
              onClick={() => {
                setTempAvatar(url);
    ...
    return (
      <>
        ...
        <div className="p-6 max-w-2xl mx-auto">
          ...
          <div className="mb-6">
            ...
            <div className="mt-4 p-4 border rounded-md bg-gray-50">
              <p className="text-base">
                <strong>Biografie:</strong> {bio || "Keine Biografie hinterlegt"}
              </p>
            </div>
            <div className="mt-4">
              <p className="text-base mb-1">
                <strong>E-Mail:</strong> {email || "Nicht gesetzt"}
              </p>
              <p className="text-base mb-1">
                <strong>Twitter:</strong> {twitter || "Nicht gesetzt"}
              </p>
              <p className="text-base mb-1">
                <strong>LinkedIn:</strong> {linkedin || "Nicht gesetzt"}
              </p>
            </div>
          </div>
          <button
          ...
          {showMiniGame && ReactDOM.createPortal(miniGameModal, document.body)}
        </div>
      </>
    );
  }
      ...
      <div className="p-6 max-w-2xl mx-auto">
        ...
        <div className="flex space-x-4 mt-4">
          ...
          <button
            ...
            className="px-6 py-3 rounded-md bg-gray-400 text-gray-800 hover:bg-gray-500"
          >
            Abbrechen
          </button>
        </div>
        {showMiniGame && ReactDOM.createPortal(miniGameModal, document.body)}
        {showAvatarModal && ReactDOM.createPortal(avatarModal, document.body)}
      </div>
    </>
  );
}
